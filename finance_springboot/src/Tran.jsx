...
function Tran() {
  ...
  const cntapi = () => {
    axios.get("http://localhost:8080/getAllTran")
      .then(response => {
  ...
  const del = (id) => {
    ...
    axios.delete(`http://localhost:8080/deleteTran/${id}`)  // ✅ Append ID in URL
      .then(response => {
  ...
  const handleSubmit = (e) => {
    ...
    if (isUpdating) {
        axios.put(`http://localhost:8080/updateTran/${dt.tid}`, dt) // 🔹 pass tid in URL
          .then(response => {
            if (response.data.status === 200) {
        ...
        axios.post("http://localhost:8080/saveTran", dt)
        .then(response => {
          if (response.data.status === 201) { // ✅ match Spring Boot's returned status
